...
class TestCephFSShell(CephFSTestCase):
    ...
    def get_cephfs_shell_script_output(self, script, mount_x=None,
                                       shell_conf_path=None, opts=None,
                                       stdin=None):
        return self.run_cephfs_shell_script(
...
class TestMisc(TestCephFSShell):
    def test_issue_cephfs_shell_cmd_at_invocation(self):
        ...
        output = self.mount_a.client_remote.run(args=['cephfs-shell', 'ls'],
...
class TestShellOpts(TestCephFSShell):
    ...
    def test_reading_conf(self):
        editorval = self.get_cephfs_shell_cmd_output('set editor ?, set editor')
        editorval = editorval.replace("'", "", 2)
        editorval = editorval.split(': ')[1]
        neweditorval = '???'

        tempconfpath = self.mount_a.client_remote.mktemp(
            suffix='cephfs-shell.conf')
        confcontents = "[cephfs-shell]\neditor =  " + neweditorval
        sudo_write_file(self.mount_a.client_remote, tempconfpath, confcontents)

        finaleditorval = self.get_cephfs_shell_cmd_output(
            cmd='set editor', shell_conf_path=tempconfpath)
        finaleditorval = finaleditorval.split(': ')[1]
        finaleditorval = finaleditorval.replace("'", "", 2)

        self.assertNotEqual(editorval, finaleditorval)

    def test_reading_conf_with_dup_opt(self):
        """
        Read conf without duplicate sections/options.
        """
        editorval = self.get_cephfs_shell_cmd_output('set editor')
        editorval = editorval.replace("'", "", 2)
        editorval = editorval.split(': ')[1]
        neweditorval = '???'
        ...
        self.assertEqual(editorval, finaleditorval)
    ...
    def test_setting_opt_after_reading_conf(self):
        ...
        confcontents = "[cephfs-shell]\neditor = " + neweditorval
        sudo_write_file(self.mount_a.client_remote, tempconfpath,confcontents)
        finaleditorval = self.get_cephfs_shell_cmd_output(cmd=('set editor %s, set '
                'editor' % (editorval)), shell_conf_path=tempconfpath)
        finaleditorval = finaleditorval.split('\n')[2]
        finaleditorval = finaleditorval.split(': ')[1]
        finaleditorval = finaleditorval.replace("'", "", 2)
