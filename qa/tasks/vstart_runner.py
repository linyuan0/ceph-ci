...
class LocalKernelMount(KernelMount):
    ...
    def mount(self, mntopts=[], createfs=True, check_status=True):
        ...
        log.info('Mounting kclient client.{} at {} {}...'.format(
                 self.client_id, self.client_remote, self.hostfs_mntpt))
        stderr = BytesIO()
        try:
...
class LocalFuseMount(FuseMount):
    def __init__(self, ctx, test_dir, client_id, client_keyring_path=None):
        super(LocalFuseMount, self).__init__(ctx=ctx, client_config=None,
            ...
            client_keyring_path=client_keyring_path, client_remote=LocalRemote())
    ...
    def mount(self, mntopts=[], createfs=True, check_status=True):
        ...
        assert self.hostfs_mntpt
        assert self.client_remote
